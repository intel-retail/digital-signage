...
## Thorough Analysis of the AIG Module

### Purpose

The `aig` (Advertise Image Generator) module is a microservice that generates advertising images dynamically using text-to-image AI models (OpenVINO GenAI). It also provides value-added image post-processing (price, promo, logo, slogan, frame overlays) and exposes REST APIs for both dynamic generation and predefined ad search.

### Main Components

- **Flask/Flask-RESTX API:** The service is a Flask app exposing REST endpoints.
- **Text-to-Image Model:** Uses OpenVINO GenAI to generate images from text prompts.
- **Image Post-Processing:** Adds price, promo, logo, slogan, and frame overlays using PIL and custom decorators.
- **Predefined Ad Search:** Stores and retrieves predefined ads, supports similarity search.

### Key Inputs

#### 1. Dynamic Ad Generation (`/minf/` endpoint, POST)

- **description** (string, required): Text prompt for image generation.
- **device** (string, required): Inference device (`CPU`, `GPU`, `NPU`).
- **price_details** (object, optional): Price overlay (text, color, alignment, etc.).
- **promo_details** (object, optional): Promo overlay (text, color, rectangle, etc.).
- **logo_details** (object, optional): Logo overlay (alignment, size, margin).
- **slogan_details** (object, optional): Slogan overlay (text, color, alignment).
- **framed_details** (object, optional): Frame overlay (activate, margin).

All these are sent as a JSON payload in a POST request.

#### 2. Predefined Ad Search/Management

- **/predefinedads/** endpoints allow:
  - Adding, updating, deleting predefined ads (with base64-encoded images and metadata).
  - Querying for the most relevant ad given a text query (returns best-matching ad).

### Key Outputs

#### 1. Dynamic Ad Generation

- **Output:** JPEG image (binary stream) with all requested overlays applied.
- **HTTP Status Codes:** 200 (success), 500 (error), 503 (busy).

#### 2. Predefined Ad Search

- **Output:** JSON with ad metadata and base64-encoded image, or direct image stream.
- **HTTP Status Codes:** 200 (success), 404 (not found), 500 (error).

### Internal Flow (Dynamic Generation)

1. **Receives POST request** with JSON payload.
2. **Validates device** and input.
3. **Loads or reuses text-to-image model** (OpenVINO GenAI).
4. **Generates image** from description.
5. **Applies overlays** (price, promo, logo, slogan, frame) in sequence using `ImgDecorator`.
6. **Returns image** as JPEG stream.

### Notable Files

- `src/server/aig_server.py`: Main Flask app and server entrypoint.
- `src/server/apis/modelinf.py`: Implements `/minf/` endpoint for dynamic ad generation.
- `src/server/apis/predefinedads.py`: Implements endpoints for predefined ad management and search.
- `src/imgproc/img_frame.py`: Image overlay and decoration utilities.
- `src/database/version.py`: Metadata and configuration for the server.
...
POST /minf/
{
  "description": "A fresh apple on a white background",
  "device": "GPU",
  "price_details": {"price": "0.99 $", "align": "right"},
  "promo_details": {"promo_text": "Buy 1 Get 1 Free"},
  "logo_details": {"align": "left"},
  "slogan_details": {"slogan_text": "Best in Town"},
  "framed_details": {"activate": true}
}
```
**Response:** JPEG image with all overlays.

### Summary Table

| Endpoint         | Method | Input Type         | Output Type         | Description                                 |
|------------------|--------|--------------------|---------------------|---------------------------------------------|
| `/minf/`         | POST   | JSON (see above)   | JPEG image stream   | Generate ad image with overlays             |
| `/predefinedads/`| GET/POST/PUT/DELETE | JSON, base64 image | JSON, image stream | Manage/search predefined ads                |

---

If you need a deeper dive into a specific function, overlay, or API, see the code or contact the maintainers.
