...
| **ASe (Advertise Searcher)**        | Retrieves and ranks relevant predefined ads from Qdrant-backed vector search.                    |
| **Web UI**                          | Displays the live video stream and current advertisements in a browser-based interface.          |

**Supporting Services:** MediaMTX (WebRTC relay), Mosquitto (MQTT broker), Qdrant (vector database), COTURN (TURN server for WebRTC).
